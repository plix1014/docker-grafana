#!/usr/bin/python3
#
# original: checkfritz.py, https://github.com/blackw1ng/FritzBox-monitor
#
# 2024-07-18: pli: added environment variables to read the credentials
#
...
FRITZBOX_HOST     = os.environ.get('FRITZBOX_HOST')

if DEBUG:
    print("DEBUG: FRITZBOX_HOST    : %s" % FRITZBOX_HOST)
    print("DEBUG: FRITZBOX_USER    : %s" % FRITZBOX_USER)
    print("DEBUG: FRITZBOX_PASSWORD: %s" % FRITZBOX_PASSWORD)


# pli: workaround: docker compose adds quotes to password string. Need to remove this string
if re.search (r'^"(.*)"', FRITZBOX_PASSWORD):
    if DEBUG:
        print("DEBUG: strip double quote from string")
    FRITZBOX_PASSWORD = FRITZBOX_PASSWORD.strip('."')
...
try:
    if DEBUG:
        print("DEBUG: fc = FritzConnection(address=%s, user=%s, password=%s, timeout=2.0)" % (FRITZBOX_HOST,FRITZBOX_USER,FRITZBOX_PASSWORD))
    fc = FritzConnection(address=FRITZBOX_HOST, user=FRITZBOX_USER, password=FRITZBOX_PASSWORD, timeout=2.0)
